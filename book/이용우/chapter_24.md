...
여기서 가지고 가야 할 핵심 키워드는 <font color='orange'>"함수가 선언된 렉시컬 환경"</font>이다.
...
따라서 `outer` 함수의 지경 변수 `x`는 더이상 유효하지 않아 보인다.
...
이를 위해 변수 `num`을 `increase` 함수의 지역 변수로 바꾸어 의도치 않은 상태 변수를 방지해해보도록 하자.
